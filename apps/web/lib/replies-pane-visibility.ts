...
/**
 * Shared visibility state for the replies sidebar on `/forum/posts/[id]`.
 * Backed by localStorage so the preference persists across reloads.
 * Cross-tab sync uses the native `storage` event; same-tab sync uses a
 * custom `lucidindex:replies-pane-visibility` event dispatched on every
 * toggle (the `storage` event doesn't fire in the originating tab).
 *
 * Default: visible = true (replies pane is open by default).
 */

import { useEffect, useState } from 'react'

const LS_KEY = 'lucidindex.replies-pane-visible'
const CUSTOM_EVENT = 'lucidindex:replies-pane-visibility'
...
export function useRepliesPaneVisibility(): { visible: boolean; toggle: () => void } {
